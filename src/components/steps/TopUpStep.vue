...
function press(digit: number) {
  ...
  if (String(next).length > MAX_DIGITS || next > maxAmount.value) {
    limitHint.value = `Höchstens ${maxAmount.value} € verfügbar.`
    return
  }
  limitHint.value = ''
  amount.value = next
...
// Auch mit der Tastatur bedienbar – nur Ziffern, kein Komma
function onKey(event: KeyboardEvent) {
  if (dialog.value?.open) return
  if (/^[0-9]$/.test(event.key)) press(Number(event.key))
  else if (event.key === 'Backspace') backspace()
  else if (event.key === 'Escape') clear()
  else if (event.key === 'Enter' && amount.value > 0) askConfirm()
}
onMounted(() => window.addEventListener('keydown', onKey))
onBeforeUnmount(() => window.removeEventListener('keydown', onKey))

function askConfirm() {
  if (amount.value <= 0 || running.value) return
  error.value = ''
  success.value = ''
